        ...
        for vehicle in self.vehicles:
            ...
            if vehicle_moves:
                if len (vehicle_moves) == 1:
                    #print(f"Vehicle {vehicle.letter} can only move in one direction: {vehicle_moves[0]}")
                    reverse_action = self.reverse_action(vehicle.letter, vehicle_moves[0])
                    valid_actions[reverse_action] = True
                else:
                    #print(f"Vehicle {vehicle.letter} can move in multiple directions: {vehicle_moves}")
